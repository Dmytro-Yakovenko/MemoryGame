...
window.addEventListener('load', (event) => {
  ...
  function startGame(){
    const shuffled =memoryGame.shuffleCards()
    let html = '';
    ...
    document.querySelectorAll('.card').forEach((card) => {
      card.addEventListener('click', () => {
        ...
        if(memoryGame.pickedCards.length>=2){
          let temp = memoryGame.checkIfPair(memoryGame.pickedCards[0].dataset.cardName, memoryGame.pickedCards[1].dataset.cardName)
